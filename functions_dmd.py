...
def dmdSpectrum(D, dt, Phi, A, s):
    ...
    #x1 = np.mean(A,axis=1)
    x1 = pandas.read_csv(r"\\nas.tu-clausthal.de\win-home$\zj19\Desktop\optdmd\error.csv", delimiter = " ", header = None).values
    ...
    np.savetxt("b_ver_real.csv", np.real(b))
    np.savetxt("b_ver_imag.csv", np.imag(b))
    # print('---------------')
    #Power Spectrum
